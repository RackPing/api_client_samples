...
# Usage: ./rp_schedule_maint.sh id 'start' 'end'
# Date: 2020 10 05
...
wget $ENABLE_DEBUG --post-data '' $options $auth_options --header='X-HTTP-Method-Override: PUT' --header="App-key: $api_key" --header="Accept-Charset: UTF-8" --header="Content-Type: application/json" "${url}/checks/$id?start_maintenance=$start&end_maintenance=$end"
ret=$?
