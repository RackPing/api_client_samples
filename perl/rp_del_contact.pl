...
# Program: rp_del_contact.pl
# Usage: ./rp_del_contact.pl id
...
# Create a request for one contact
   my $req = HTTP::Request->new(DELETE => $url . "/contacts/$contact_id");

# Pass request to the user agent and get a response back
   my $res = $ua->request($req);

   print STDERR "info: delete one contact\n" if $DEBUG;
