...
// Program: go_resume_maint.go
// Usage: go run rp_resume_maint.go id
...
func main() {
...

   {
      ...
      fmt.Printf("%s\n", "Disable maintenance for one check")

      req, err := http.NewRequest("PUT", url + "/checks/"+id+"?maintenance=0", nil)
      req.SetBasicAuth(g_username, g_password)
