...
old_id=`./rp_list_contacts | jq '.contacts[] | select(.last=="Doe") | .id'`
if [[ $old_id =~ $re ]] ; then
   ./rp_del_contact $old_id
fi

# support versions of jq before 1.5, which don't have startswith()
old_id=`./rp_list_checks | jq '.checks[] | select(.name=="APITest" or .name=="APITestTest") | .id'`
if [[ $old_id =~ $re ]] ; then
   ./rp_del_check $old_id
fi
...
id=`./rp_add_contact | $jq_cmd '.contact .id'`
...
id=`./rp_add_check | $jq_cmd '.checks .id'`
...
./rp_pause_maint $id
./rp_resume_maint $id
./rp_schedule_maint $id "2020-10-01 00:00:00" "2020-10-07 00:00:00"
