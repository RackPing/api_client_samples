...
fn main() {
    ...
    let url        = scheme.to_string() + &domain.to_string() + &base_url.to_string() + "/checks/" + id + "?maintenance=0";
...
let custom = reqwest::redirect::Policy::custom(|attempt| {
    ...
    println!("info: end maintenance on one check\n");
