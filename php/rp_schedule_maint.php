#!/usr/bin/php
<?php
// Program: rp_schedule_maint.php
// Usage: ./rp_schedule_maint.php id 'start' 'end'
// Date: 2020 10 07
...
function do_curl($method, $endpoint, $user, $pw, $api_key, $timeout, $data) {
   ...
   if ($argc < 3) {
      echo "usage: ./$argv[0] contactid 'start' 'end'\n";
      exit(1);
   }

   $id    = $argv[1];
   $start = str_replace(" ","%20",$argv[2]);
   $end   = str_replace(" ","%20",$argv[3]);

   $ret = 0;

   echo "Enable maintenance window for one check\n";
   $response = do_curl('PUT', $url . '/checks/' . $id . '?start_maintenance=' . $start . '&end_maintenance=' . $end, $user, $password, $api_key, $timeout, NULL);
