# Program: rp_pause_check.ps1
# Usage: rp_pause_check.ps1
...
if (!$id) {
   Write-Error "usage: rp_pause_check.ps1 id"
   exit
}
...
$url = $env:RP_SCHEME + $env:RP_DOMAIN + $env:RP_BASE_URL + "/checks/" + $id + '?paused=1'
...
Write-Error "info: pause one check"
