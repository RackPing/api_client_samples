...
old_id=`$cmd ./rp_list_contacts.$mylang | jq '.contacts[] | select(.last=="Doe") | .id'`
if [[ $old_id =~ $re ]] ; then
   $cmd ./rp_del_contact.$mylang $old_id
fi

# support versions of jq before 1.5, which don't have startswith()
old_id=`$cmd ./rp_list_checks.$mylang | jq '.checks[] | select(.name=="APITest" or .name=="APITestTest") | .id'`
if [[ $old_id =~ $re ]] ; then
   $cmd ./rp_del_check.$mylang $old_id
fi

# 1. Demo the contacts (users) scripts

$cmd ./rp_list_contacts.$mylang

# edit rp_add_contact.$mylang to set the contact (user) info
id=`$cmd ./rp_add_contact.$mylang | $jq_cmd '.contact .id'`
...
$cmd ./rp_update_contact.$mylang $id

$cmd ./rp_del_contact.$mylang $id

$cmd ./rp_list_contacts.$mylang

# 2. Demo the checks (monitors) scripts

$cmd ./rp_list_checks.$mylang

# edit rp_add_check.$mylang to set the check (monitor) info
id=`$cmd ./rp_add_check.$mylang | $jq_cmd '.checks .id'`
...
$cmd ./rp_update_check.$mylang $id

$cmd ./rp_pause_check.$mylang $id
$cmd ./rp_resume_check.$mylang $id
