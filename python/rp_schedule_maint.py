...
# Program: rp_schedule_maint.py
# Usage: ./rp_schedule_maint.py id 'start' 'end'
# Version: 1.0
...
if len(sys.argv) < 4:
   sys.exit('usage: ' + sys.argv[0] + ' id "start" "end"')
...
monitor = sys.argv[1]
start   = sys.argv[2].replace(" ", "%20");
end     = sys.argv[3].replace(" ", "%20");
...
def main():
    ...
    try:
        print("info: schedule maintenance window for one check:\n")
